...
PTable::~PTable() {
    if (bm != 0)
        delete bm;
    // for (int i = 0; i < psize; i++)	// for some reasons, trying to delete
    // keeps throwing
    //    if (pcb[i]) delete pcb[i];	// segmentation fault. I was unable to
    //    pinpoint the
...
int PTable::ExecUpdate(char *name) {
    ...
    if (name == NULL // if the program doesn't exist
        ...
        || strcmp(name, "./test/scheduler") ==
               0 // if the program is the main process
        || strcmp(name, currentThread->getName()) ==
               0) { // if the program is the thread itself
        printf("\n\n Unable to execute.");
        bmsem->V();
        return -1;
    }
...
int PTable::ExitUpdate(int ec) {
    ...
    pcb[id]
        ->JoinRelease(); // Free the parent process that are waiting (if exists)
...
int PTable::JoinUpdate(int id) {
    if (id < 0 // if the process doesn't exist or if the caller is not
        || currentThread->processID !=
               pcb[id]->parentID) { // the parent of the process

...
class Machine {
  public:
    ...
    // Routines for copying memory space from UserSpace to SystemSpace
    // (KernelSpace) and vice versa

    char *User2System(int virtAddr, int limit);
    // Copy buffer from user memory space to system
    // memory space. Take in userspace address (virtAddr)
    // and buffer limit. Return buffer in system space.
    int System2User(int virtAddr, int len, char *buffer);
    // Copy buffer from system memory space to user
    // memory space. Take in userspace address (virtAddr),
    // buffer limit and the buffer in system space. Return

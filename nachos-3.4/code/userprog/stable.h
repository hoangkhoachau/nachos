...
class STable {
  public:
    ...
    int Create(char *name,
               int init); // Create a new semaphore with name if hasn't existed
    int
    Wait(char *name); // Put the semaphore with name into wait mode if it exists
    int Signal(
        char *name); // Send signal to free the semaphore with name if it exists
